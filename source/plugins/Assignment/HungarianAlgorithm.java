...
import java.util.*;
//This implementation has two potential problems. The first one is that it goes into infinite loop when all rows are covered, because
//it only checks whether all collumns are covered. The second problem is that when the number of columns re bigger than number of rows,
//the assignment of the matrix retval will have problem.
//This code also can not apply to matrix of single column (row as well I guess). Added a routine to square the matrix. TJ 9/31/2011
/**
...
public class HungarianAlgorithm implements AssignmentAlgorithm {
    ...
    int m_nRows,m_nColumns,nDim;

    public int[][] computeAssignments(float[][] matrix) {


        // subtract minumum value from rows and columns to create lots of zeroes
        m_nRows=matrix.length;
        m_nColumns=matrix[0].length;
        nDim=Math.max(m_nRows, m_nColumns);
        if(m_nRows!=m_nColumns) matrix=square(matrix);
        reduceMatrix(matrix);
        ...
        while (!allAreCovered(coveredCols)) {
            ...
            while (primedZero == null) {
               ...
               makeMoreZeroes(matrix,coveredRows,coveredCols);
                ...
                iterations++;
                if(iterations>5000) terminate=true;
                if(terminate) {
                    return null;
                }
        ...
        int[][] retval = new int[m_nRows][];//
        int nPairs=0,row;//tj
        for (int i = 0; i < m_nColumns;  i++) {
            if(starsByCol[i]>=0){
                row=starsByCol[i];
                if(row<m_nRows){
                    retval[nPairs]=new int[2];
                    retval[nPairs][0]=row;//tj modified to handled the case of rows<cols
                    retval[nPairs][1]=i;
                    nPairs++;
                }
            }
    ...
    float[][] square(float[][] matrix){
        int i,j;
        float sum=0;
        float mt[][]=new float[nDim][nDim];
        for(i=0;i<m_nRows;i++){
