...
public class ImageCanvas extends Canvas implements MouseListener, MouseMotionListener, Cloneable {
    ...
    public void paint(Graphics g) {
		...
		try {
			...
			if (IJ.debugMode) showFrameRate(g);
                ...
                firePaintNotifyingEvent();
    }
    //this block is added by TJ on 11/20/2011
    ArrayList <ActionListener> m_cvPaintListener;
    public void initPaintListeners(){
        if(m_cvPaintListener==null)m_cvPaintListener=new ArrayList();
    }
    void firePaintNotifyingEvent(){
        if(m_cvPaintListener==null)m_cvPaintListener=new ArrayList();
        int len=m_cvPaintListener.size(),i;
        ActionListener al;
        ActionEvent ae=new ActionEvent(this,0,"Painted");
        for(i=0;i<len;i++){
            al=m_cvPaintListener.get(i);
            if(al==null) continue;
    ...
    public void addPaintListener(ActionListener al){
        if(m_cvPaintListener==null) m_cvPaintListener=new ArrayList();
        removePaintListener(al);
        m_cvPaintListener.add(al);
    }
    public void removePaintListener(ActionListener al0){
        if(m_cvPaintListener!=null){
            int len=m_cvPaintListener.size(),i;
            ActionListener al;
            for(i=len-1;i>=0;i--){
                al=m_cvPaintListener.get(i);
                if(al==al0) m_cvPaintListener.remove(i);
            }
        }
    }
