 ...
 */   
public final class Geometry_Geo
{
  /**
                                             ...
                                             double fractionFrom0)
  {
    ...
    double[] p = Geometry_Geo.computePointOnLine (p0, p1, fractionFrom0);
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength,
                                 double anchor)
  {
    double[] p = Geometry_Geo.computePointOnLine (p0, p1, anchor);
    
    double length0 = toLength * anchor;
    double length1 = toLength * (1.0 - anchor);
    
    Geometry_Geo.extendLine (p, p0, length0);
    Geometry_Geo.extendLine (p, p1, length1);
  }
  ...
  public static void extendLine (double[] p0, double[] p1, double toLength)
  {
    double oldLength = Geometry_Geo.length (p0, p1);
  ...
  {
    double[] v = Geometry_Geo.createVector (p0, p1);
    return length (v);
  }
  ...
  {
    return Geometry_Geo.length ((double) x0, (double) y0,
                            (double) x1, (double) y1);
  }
  ...
  public static double length (int[] x, int[] y, boolean isClosed)
  {
    ...
    for (int i = 0; i < nPoints-1; i++)
      length += Geometry_Geo.length (x[i], y[i], x[i+1], y[i+1]);

    // Add last leg if this is a polygon
    if (isClosed && nPoints > 1)
      length += Geometry_Geo.length (x[nPoints-1], y[nPoints-1], x[0], y[0]);
                                 ...
                                 int x, int y)
  {
    ...
    double length = Geometry_Geo.length (x0, y0, x1, y1);
    if (length == 0.0)
      return Geometry_Geo.length (x0, y0, x, y);
    ...
    length = Geometry_Geo.length (xp, yp, x, y);
    return length;
  }
  ...
  {
    double[] v0 = Geometry_Geo.createVector (p0, p1);
    double[] v1 = Geometry_Geo.createVector (p0, p2);

    double dotProduct = Geometry_Geo.computeDotProduct (v0, v1);

    double length1 = Geometry_Geo.length (v0);
    double length2 = Geometry_Geo.length (v1);
  ...
  {
    int s1 = Geometry_Geo.sameSide (x0, y0, x1, y1, x2, y2, x3, y3);
    int s2 = Geometry_Geo.sameSide (x2, y2, x3, y3, x0, y0, x1, y1);
                                                     ...
                                                     int x1, int y1)
  {
    ...
    if (Geometry_Geo.isPointInsideRectangle (x0, y0, x1, y1, lx0, ly0) ||
        Geometry_Geo.isPointInsideRectangle (x0, y0, x1, y1, lx1, ly1))
      return true;

    // If it intersects it goes through. Need to check three sides only.

    // Check against top rectangle line
    if (Geometry_Geo.isLineIntersectingLine (lx0, ly0, lx1, ly1,
                                         x0, y0, x1, y0))
      return true;
                                                         ...
                                                         int x1, int y1)
  {
    ...
    if (Geometry_Geo.isPointInsideRectangle (x[0], y[0], x0, y0, x1, y1))
      return true;
    ...
    for (int i = 1; i < x.length; i++) {
      if (x[i-1] != x[i] || y[i-1] != y[i])
        if (Geometry_Geo.isLineIntersectingRectangle (x[i-1], y[i-1],
                                                  x[i], y[i],
                                                  x0, y0, x1, y1))
          return true;
    }
                                                        ...
                                                        int x1, int y1)
  {
    ...
    if (n == 1)
      return Geometry_Geo.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]);
    
    //
    // If the polyline constituting the polygon intersects the rectangle
    // the polygon does too.
    //
    if (Geometry_Geo.isPolylineIntersectingRectangle (x, y, x0, y0, x1, y1))
      return true;
  
    // Check last leg as well
    if (Geometry_Geo.isLineIntersectingRectangle (x[n-2], y[n-2], x[n-1], y[n-1],
                                              x0, y0, x1, y1))
      return true;
    ...
    // or separate.
    //
    if (Geometry_Geo.isPointInsidePolygon (x, y, x0, y0) ||
        Geometry_Geo.isPointInsideRectangle (x0, y0, x1, y1, x[0], y[0]))
      return true;
  ...
  public static double[] computePolygonCentroid (double[] x, double[] y)
  {
    ...
    double area = Geometry_Geo.computePolygonArea (x, y);
                                                 ...
                                                 double[] intersection)
  {
    ...
    double a0 = Geometry_Geo.equals (x0, x1, LIMIT) ?
                INFINITY : (y0 - y1) / (x0 - x1);
    double a1 = Geometry_Geo.equals (x2, x3, LIMIT) ?
    ...
    if (Geometry_Geo.equals (a0, a1)) {
      if (!Geometry_Geo.equals (b0, b1))
        return -1; // Parallell non-overlapping
      
      else {
        if (Geometry_Geo.equals (x0, x1)) {
          if (Math.min (y0, y1) < Math.max (y2, y3) ||
              Math.max (y0, y1) > Math.min (y2, y3)) {
            double twoMiddle = y0 + y1 + y2 + y3 -
                               Geometry_Geo.min (y0, y1, y2, y3) -
                               Geometry_Geo.max (y0, y1, y2, y3);
            y = (twoMiddle) / 2.0;
            x = (y - b0) / a0;
          }
          ...
          if (Math.min (x0, x1) < Math.max (x2, x3) ||
            ...
            double twoMiddle = x0 + x1 + x2 + x3 -
                               Geometry_Geo.min (x0, x1, x2, x3) -
                               Geometry_Geo.max (x0, x1, x2, x3);
            x = (twoMiddle) / 2.0;
            y = a0 * x + b0;
          }
          else return -1;
    ...
    if (Geometry_Geo.equals (a0, INFINITY)) {
      x = x0;
      y = a1 * x + b1;
    }
    else if (Geometry_Geo.equals (a1, INFINITY)) {
      x = x2;
      y = a0 * x + b0;
    }
    else {
    ...
    if (Geometry_Geo.equals (x0, x1)) {
      if (y0 < y1)
        distanceFrom1 = y < y0 ? Geometry_Geo.length (x, y, x0, y0) :
                        y > y1 ? Geometry_Geo.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = y < y1 ? Geometry_Geo.length (x, y, x1, y1) :
                        y > y0 ? Geometry_Geo.length (x, y, x0, y0) : 0.0;
    }
    else {
      if (x0 < x1)
        distanceFrom1 = x < x0 ? Geometry_Geo.length (x, y, x0, y0) :
                        x > x1 ? Geometry_Geo.length (x, y, x1, y1) : 0.0;
      else
        distanceFrom1 = x < x1 ? Geometry_Geo.length (x, y, x1, y1) :
                        x > x0 ? Geometry_Geo.length (x, y, x0, y0) : 0.0;
    }
    
    double distanceFrom2;
    if (Geometry_Geo.equals (x2, x3)) {
      if (y2 < y3)
        distanceFrom2 = y < y2 ? Geometry_Geo.length (x, y, x2, y2) :
                        y > y3 ? Geometry_Geo.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = y < y3 ? Geometry_Geo.length (x, y, x3, y3) :
                        y > y2 ? Geometry_Geo.length (x, y, x2, y2) : 0.0;
    }
    else {
      if (x2 < x3)
        distanceFrom2 = x < x2 ? Geometry_Geo.length (x, y, x2, y2) :
                        x > x3 ? Geometry_Geo.length (x, y, x3, y3) : 0.0;
      else
        distanceFrom2 = x < x3 ? Geometry_Geo.length (x, y, x3, y3) :
                        x > x2 ? Geometry_Geo.length (x, y, x2, y2) : 0.0;
    }
    
    return Geometry_Geo.equals (distanceFrom1, 0.0) &&
      Geometry_Geo.equals (distanceFrom2, 0.0) ? 1 : 0;
  }
                                                       ...
                                                       double x1, double y1)
  {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      if (Geometry_Geo.equals (x2, x3) && Geometry_Geo.equals (y2, y3))
        continue;

      int type = Geometry_Geo.findLineSegmentIntersection (x0, y0, x1, y1,
                                                       x2, y2, x3, y3,
                                                       intersection);
      ...
      if (type == -2) { // Overlapping
        ...
        int side = Geometry_Geo.sameSide (x0, y0, x1, y1,
                                      x[p1], y[p1], x[p2], y[p2]);
  ...
  public static int[] createEllipse (int[] x, int[] y)
  {
    ...
    double axisAngle = Geometry_Geo.computeAngle (p0, p1, p2);
  
    // dx / dy  
    double dx = Geometry_Geo.length (x0, y0, x[1], y[1]);
    double dy = Geometry_Geo.length (x0, y0, x[0], y[0]) * Math.sin (axisAngle);
    ...
    // it to zero as well.
    if (!Geometry_Geo.equals (axisAngle, Math.PI/2.0, 0.1) && 
        !Geometry_Geo.equals (axisAngle, Math.PI,     0.1) &&
        !Geometry_Geo.equals (axisAngle, 0.0,         0.1)) {
                                   ...
                                   double length, double angle, double inset)
  {
    ...
    int[] pos1 = new int[2];
    Geometry_Geo.findPolygonPosition (x, y, length, pos1);
                                             ...
                                             int[] position)
  {
    ...
    for (int i = 1; i < x.length; i++) {
      double legLength = Geometry_Geo.length (x[i-1], y[i-1], x[i], y[i]);
