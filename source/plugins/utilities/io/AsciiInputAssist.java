...
public class AsciiInputAssist {
    ...
    public static void  getCSVFileContentsAsStringArrayList(String path, ArrayList<String[]> contents){
        String sExt=FileAssist.getFileExtention(path);        
        if(sExt.equalsIgnoreCase("csv")){
            contents.clear();
            ArrayList<String> lines=new ArrayList();
            getFileContentsAsStringArray(path,lines);
            int i,j,len=lines.size();
            ...
            for(i=0;i<len;i++){
                ...
                psLine=new String[num];
                for(j=0;j<num;j++){
                    psLine[j]=stk.nextToken();
                }
                contents.add(psLine);
            }
        }else{
            IJ.error(path+"is not a csv file");
        }
    }
