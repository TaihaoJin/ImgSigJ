...
public class PlotWindow extends ImageWindow implements ActionListener, ClipboardOwner {
	...
	private Plot plot;
        boolean displayXAxisLabel;//added by Taihao Jin on 10/5/2014
        protected int TOP_MARGIN=15, BOTTOM_MARGIN=40, ExtraTopMargin=0,ExtraBottomMargin=0;//added by TJ 9/21/2014
	...
	*/
	...
	static ImagePlus createImage1(String title, String xLabel, String yLabel, float[] xValues, float[] yValues,int tmar,int bmar) {//chaged by TJ on 14921
		staticPlot = new Plot(title, xLabel, yLabel, xValues, yValues, tmar,bmar);
		return new ImagePlus(title, staticPlot.getBlankProcessor());
	}
...
//		if (plot!=null && plot.frame!=null && coordinates!=null)//modification TJ 6/5/2012
//			coordinates.setText(plot.getCoordinates(x,y));
	...
	public String getCoordinate(int x, int y){
            return plot.getCoordinates(x, y);
        }
	...
	public String getPoingCoordinate(int x, int y){//x and y are the coordinate a point on the plot image (the cursor location), it returns the data value
            return plot.getPointCoordinates(x, y);
        }        
        public Point getPixelCoordinates(double x, double y){//the pixel coordinate where the data point (x,y) should be drawn
            return plot.getPixelCoordinates(x,y);
        }
        public Rectangle getPlotFrame(){//TJ 11/17/2011
            return plot.getPlotFrame();
        }
        public void displayXAxisLabel(boolean show){
            displayXAxisLabel=show;
        }
	public void updatePlot(String title, String xLabel, String yLabel, float[] xValues, float[] yValues) {
//            updateImage(createImage(title, xLabel, yLabel, xValues, yValues));
            ImagePlus impl=getImagePlus();
            Roi roi=null;
            if(impl!=null) roi=impl.getRoi();
            impl=createImage1(title, xLabel, yLabel, xValues, yValues,TOP_MARGIN+ExtraTopMargin,BOTTOM_MARGIN+ExtraBottomMargin);//chaaged by TJ on 14921
//            impl=createImage(title, xLabel, yLabel, xValues, yValues);
            if(roi!=null) impl.setRoi(roi);
            setImagePlus(impl);
            updateImage(getImagePlus());
            plot = staticPlot;
	}
	public void drawPlot() {
                plot.showXAxisLabel(displayXAxisLabel);
		plot.draw();
		ImageProcessor ip = plot.getProcessor();
		if ((ip instanceof ColorProcessor) && (imp.getProcessor() instanceof ByteProcessor))
			imp.setProcessor(null, ip);
        ...
        public void setButtons(Button list, Button save, Button copy, Label coordinates){
            this.list=list;
            this.save=save;
            this.copy=copy;
            this.coordinates=coordinates;
        }
        public int getCoordinate(int x, int y, double[] pdXY){
            return plot.getPointCoordinates(x, y, pdXY);
        }
        public void getScales(double[] scales){
            plot.getScales(scales);
        }
        public void setExtraTopMargin(int mar){
            ExtraTopMargin=mar;
        }
        public void setExtraBottomMargin(int mar){
            ExtraBottomMargin=mar;
        }
}
