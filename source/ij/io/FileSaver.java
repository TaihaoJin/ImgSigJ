...
public class FileSaver {
	...
	/** Resaves the image. Calls saveAsTiff() if this is a new image, 
         * not a TIFF, a 
		stack or if the image was loaded using a URL. Returns false 
         * if saveAsTiff() is
		called and the user selects cancel in the file save 
         * dialog box. */
	public boolean save() {
		FileInfo ofi = null;
		if (imp!=null) ofi = imp.getOriginalFileInfo();
		boolean validName = ofi!=null && 
                        imp.getTitle().equals(ofi.fileName);
		if (validName && ofi.fileFormat==FileInfo.TIFF && 
                        imp.getStackSize()==1 && ofi.nImages==1 && 
			...
			if (!IJ.isMacro() && f!=null && f.exists()) {
				if (!IJ.showMessageWithCancel("Save as TIFF", 
                                        "The file "+ofi.fileName+" already " +
	...
	String getPath(String type, String extension) {
		name = imp.getTitle();
		SaveDialog sd = new SaveDialog("Save as "+type, name, 
                        extension);
		name = sd.getFileName();
		if (name==null)
			return null;
	...
	public boolean saveAsTiffStack(String path) {
		...
		if (fi.pixels==null && imp.getStack().isVirtual())
			{IJ.error("Save As Tiff", "Virtual stacks not " +
                                 "supported."); return false;}
		Object info = imp.getProperty("Info");
		if (info!=null && (info instanceof String))
			fi.info = (String)info;
//		fi.description = getDescriptionString();
		fi.description += getDescriptionString();//TJ modified from the above line on 08923
	...
	/** Save the image or stack in TIFF/ZIP format 
	...
	public static boolean okForGif(ImagePlus imp) {
		...
		if (type==ImagePlus.COLOR_RGB) {
			IJ.error("To save as Gif, the image must be " +
	...
	*/
	public boolean saveAsJpeg(String path) {
		...
		if (!(imp.getType()==ImagePlus.GRAY16 || imp.getType()==
                        ImagePlus.GRAY32))
			updateImp(fi, fi.GIF_OR_JPG);
		return true;
	}
	...
	*/
	...
	public String getDescriptionString() {
		...
		if (fi.valueUnit!=null && fi.calibrationFunction!=
                        Calibration.CUSTOM) {
		...
		int type = imp.getType();
		boolean enhancedLut = (type==ImagePlus.GRAY8 || 
                        type==ImagePlus.COLOR_256) && (min!=0.0 || max !=255.0);
		if (enhancedLut || type==ImagePlus.GRAY16 || 
