...
public class PluginClassLoader extends ClassLoader {
	...
	void addJars_TJ(String path) {
        ...
        void checkSubdirectories_TJ(File f)
        {
		String[] innerlist = f.list();
		for (int j=0; j<innerlist.length; j++) {
                    File f1=new File(f.getPath(), innerlist[j]);
                    if (f1.isDirectory()) {
                             checkSubdirectories_TJ(f1);
		} else 
				addJAR(f1);
            }
    ...
    private byte [] loadIt_TJ(String path, String classname) {
        ...
        for(int i=0;i<list.length;i++){
            ...
            if(f1.isDirectory())
            {
                classBytes=loadIt_TJ(f1.getPath(), classname);
    ...
    protected byte[] loadClassBytes(String name) {
        byte [] classBytes = null;
        classBytes = loadIt_TJ(path, name);
        if (classBytes == null) {
