 ...
 *  case it is "stuck" in a local minimum (by default, restarted once).  Also
 * includes
 *  settings dialog option for user control over simplex parameters and
 * functions to
 ...
 *
 *	2008/01/21: Modified to do Gaussian fitting by Stefan Wörz (s.woerz at
 * dkfz.de).
 *
 */
public class CurveFitter {    
    ...
    // numParams+1 (includes sumLocalResiduaalsSqrd)
    private int worst;			// worst current parametre estimates
    private int nextWorst;
    // 2nd worst current parametre estimates
    private int best;			// best current parametre estimates
    private double[][] simp; 		
    // the simplex (the last element of the array at each vertice is
    ...
    // number of times to restart simplex after first soln.
     ...
     *          doFit(fitType, true) pops up a dialog
     *          allowing control over simplex parameters
    ...
    public void doFit(int fitType, boolean showSettings) {
        ...
        while (!done) {
            ...
            double rtol = 2 * Math.abs(simp[best][numParams] -
                    simp[worst][numParams]) /
            (Math.abs(simp[best][numParams]) + Math.abs(simp[worst][numParams])
                    + 0.0000000001);
    ...
    void initialize() {
        ...
        // need 1 more vertice than parametres,
        ...
        switch (fit) {
            ...
            case GAMMA_VARIATE:
                ...
                //  t0 [b] = time of first rise in gamma curve -
                //so use the user specified first limit
                //  tm = t0 + a*B [c*d] where tm is the time of
                ...
                simp[0][1] = yData[getMax(yData)] / (Math.pow(ab,
                        simp[0][2]) * Math.exp(-ab/simp[0][3]));
                break;
            case LOG2:
                simp[0][0] = 0.5;
                simp[0][1] = 0.05;
    ...
    private void settingsDialog() {
        ...
        char pChar = 'a';
        for (int i = 0; i < numParams; i++) {
            gd.addNumericField("Initial "+(new Character(pChar)).toString()+
                    ":", simp[0][i], 2);
            pChar++;
        }
        gd.addNumericField("Maximum iterations:", maxIter, 0);
        gd.addNumericField("Number of restarts:", restarts, 0);
        gd.addNumericField("Error tolerance [1*10^(-x)]:", -
                (Math.log(maxError)/Math.log(10)), 0);
        gd.showDialog();
    ...
    void restart(int n) {
        ...
        sumResiduals(simp[0]);
        // Get sum of residuals^2 for first vertex
        double[] step = new double[numParams];
        for (int i = 0; i < numParams; i++) {
            step[i] = simp[0][i] / 2.0;     // Step half the parametre value
            if (step[i] == 0.0)
                // We can't have them all the same or we're going nowhere
                step[i] = 0.01;
        }
        ...
        for (int i = 0; i < numParams; i++) {
            p[i] = step[i] * (Math.sqrt(numVertices) +
                    numParams - 1.0)/(numParams * root2);
    ...
    public static double f(int fit, double[] p, double x) {
        ...
        switch (fit) {
            ...
            case GAUSSIAN:
                return p[0]+
                (p[1]-p[0])*Math.exp(-(x-p[2])*(x-p[2])/(2.0*p[3]*p[3]));
            case POWER:
                if (x == 0.0)
                    return 0.0;
                else
    ...
    */
    ...
    public double getFitGoodness() {
        ...
        if (sumMeanDiffSqr > 0.0 && degreesOfFreedom != 0)
            fitGoodness = 1.0 - (getSumResidualsSqr() / degreesOfFreedom) *
                    ((numPoints) / sumMeanDiffSqr);
    ...
    void sumResiduals (double[] x) {
        ...
        for (int i = 0; i < numPoints; i++) {
            ...
            //        if (IJ.debugMode) ij.IJ.log(i+" "+x[n-1]+" "+
	...
	/** Sets the initial parameters, which override the
