 ...
 * @author Taihao
 */
public class LevelTransitionDetector_Downward {
    ArrayList<Double> dvXs,dvYs;
//    double[] m_pdY,m_pdX;
    int m_nMaxLevels, m_nMinSegLength;
    ArrayList<Integer> m_nvLx, m_nvLn, m_nvTransitionPositions;

    ArrayList<Integer> m_nvTransitions;
    double[] m_pdLx,m_pdRx;
    int m_nNumLocalMaxima,m_nData,m_nRulerSize;
    ArrayList<IPOGTLevelNode> m_cvLevelNodes;
    double m_dThreshold,m_dRisingInterval;
    ArrayList<Double> m_dvX,m_dvY,m_dvYo,m_dvTransitionPositions,m_dvP,m_dvP1;
    int[] m_pnLxPositions,m_pnLnPositions;
    ArrayList<PlotWindow> m_cvTransitionPlotWindow;
    boolean showTransitions;


    public LevelTransitionDetector_Downward(){
        m_dThreshold=0.00001;
    }
/*
//    public LevelTransitionDetector_Downward(double[] pdX,double[] pdY, int nMaxLevels, int nRisingInterval){
    public LevelTransitionDetector_Downward(ArrayList<Double> dvX, ArrayList<Double> dvY, int nMaxLevels, double dRisingInterval, ArrayList<PlotWindow> pws){
...
//        m_pdX=pdX;
        ...
        m_nRulerSize=5*m_nMinSegLength;
    ...
    int nextIndex(int index, int sign){
...
//        QuickSortInteger.quicksort(m_nvTransitions);
    ...
    boolean isValidTransition(int left, int index, int right){
        ArrayList <Double> dvL=getLinearDriftCorrectedData(nextIndex(left,1),index,index);
        int indext=nextIndex(index,1);
        ArrayList <Double> dvR=getLinearDriftCorrectedData(indext,right,indext);
        if(dvL.isEmpty()||dvR.isEmpty()) return false;
        double p=HypothesisTester.chiSquareTestDataSetsComparison(dvL, dvR);
        double p1=HypothesisTester.tTestGivenSD(dvL, dvR);
//        CommonStatisticsMethods.addToArray(dvL, 50);
//        double pt=HypothesisTester.chiSquareTestDataSetsComparison(dvL, dvR);
//        double pt1=HypothesisTester.tTestGivenSD(dvL, dvR);
        if(p1<m_dThreshold){
            m_dvP.add(p);
            m_dvP1.add(p1);
        }
