...
public class GraphicalObject {
    ...
    ArrayList <GraphicalObject> enclosedObjects;//Topological consideration, an object can enclose other objects.
    ...
    ArrayList <GraphicalObject> enclosingObjects;//objects that enclose the object.
    ArrayList <GraphicalObject> neighboringObjects;//objects that contact with the object.
    GraphicalObjectsHandler GOHandler;//the graphical object handler that owns this object.
    ...
    public boolean equals(GraphicalObject go){
    ...
    public GraphicalObject(GraphicalObjectProperty type0){
        this();
        type=type0;
        int rgb[]=new int[3];
        rgb[0]=type.red;
    ...
    public GraphicalObjectProperty getType(){
        return new GraphicalObjectProperty(type);
    }
    public void mergeGO(GraphicalObject go){
    ...
    public void setGOPType(GraphicalObjectProperty gop){
        type=new GraphicalObjectProperty(gop);
    }
    ...
    public void drawContour(ImagePlus impl, boolean displayCurvature){        
        ...
        if(displayCurvature&&size1>0){
            ...
            minCurvature=Double.POSITIVE_INFINITY;
            double c=0;
...
//                p1=p2;
    ...
    public void buildEnclosedObjectList(int w0, int h0){
        ...
        if(enclosingObjects!=null) enclosingObjects.clear();
            ...
            enclosingObjects=new ArrayList <GraphicalObject> ();
...
//                pixels[y-yMin+1][x-xMin+1]=(byte)150;
                ...
                moveType=1;
                moveSum+=moveType;
//                if(GOHandler.performedEdgeFinding()&&go.type.StructureId==-2){//go is an edge.
                if(go.type.name.equalsIgnoreCase("edge")){//go is an edge.
...
//        CommonMethods.displayAsImage(title,w, h, pixels);
        if(!this.isCropped(w0, h0)){
            if(size==1){
                ...
                if(go.enclosedObjects==null) go.enclosedObjects=new ArrayList<GraphicalObject>();
...
//        if(Contour.size()>4) minContour=CommonMethods.getMinimumVertexPolygon(Contour);
 ...
 //      if(!bShow) return false;
    ...
    public void buildBounds(){
        bounds.x=xRange.getMin();
        bounds.y=yRange.getMin();
        bounds.height=yRange.getMax()-yRange.getMin();
        bounds.width=xRange.getMax()-xRange.getMin();
    }
    public void markObject(ImagePlus impl){
        int w=impl.getWidth(),h=impl.getHeight();
        int[] xPoints={xRange.getMin(),xRange.getMax(),xRange.getMax(),xRange.getMin()};
        int[] yPoints={yRange.getMin(),yRange.getMin(),yRange.getMax(),yRange.getMax()};
        int i;
        for(i=0;i<4;i+=3){
            if(xPoints[i]>0)xPoints[i]--;
        }
        for(i=1;i<3;i++){
            if(xPoints[i]<w-2)xPoints[i]++;
        }
        for(i=0;i<2;i++){
            if(yPoints[i]>0)yPoints[i]--;
        }
        for(i=2;i<4;i++){
            if(yPoints[i]<h-2)yPoints[i]++;
        }
        int x0=xPoints[3],y0=yPoints[3],x,y;
        for(i=0;i<4;i++){
            x=xPoints[i];
            y=yPoints[i];
            impl.getProcessor().drawLine(x0, y0, x, y);
            x0=x;
            y0=y;
        }
    }
