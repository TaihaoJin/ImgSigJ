...
public class ByteConverter {
    public static void getIntArray(byte[] bts, int offset, int len, int[] pnInts, int offsetI, int lenI){
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        IntBuffer ib=bf.asIntBuffer();
        ib.get(pnInts, offsetI, lenI);
    }
    public static void getFloatArray(byte[] bts, int offset, int len, float[] pfData, int offsetI, int lenI){
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        FloatBuffer ib=bf.asFloatBuffer();
        ib.get(pfData, offsetI, lenI);
    }
    public static void getDoubleArray(byte[] bts, int offset, int len, double[] pdData, int offsetI, int lenI){
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        DoubleBuffer ib=bf.asDoubleBuffer();
        ib.get(pdData, offsetI, lenI);
    }
    public static void getShortArray(byte[] bts, int offset, int len, short[] pnData, int offsetI, int lenI){
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        ShortBuffer ib=bf.asShortBuffer();
        ib.get(pnData, offsetI, lenI);
    }
    public static void getShortArray_LittleEndian(byte[] bts, int offset, int len, short[] pnData, int offsetI, int lenI){
        int unitLength=2;
        int num=len/unitLength;
        reverseEndianness(bts, offset, unitLength, num);
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        ShortBuffer ib=bf.asShortBuffer();
        ib.get(pnData, offsetI, lenI);
    }
    public static int toInteger(byte[] bts, int offset, int len){
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        return bf.getInt();
    }
    public static float toFloat(byte[] bts, int offset, int len){
        ByteBuffer bf=ByteBuffer.wrap(bts, offset, len);
        return bf.getFloat();
    }
    public static void reverseEndianness(byte[] bts0, int offset0, int unitLength, int num){
        byte bts[]=new byte[unitLength];
        int offset=offset0, i,j,index;
        for(i=0;i<num;i++){
            for(j=0;j<unitLength;j++){
                bts[j]=bts0[offset+j];
            }
            for(j=0;j<unitLength;j++){
                index=unitLength-1-j;
                bts0[offset+j]=bts[index];
            }
            offset+=unitLength;
        }
    }
    //the following is from http://www.daniweb.com/software-development/java/code/216874
/* ========================= */
