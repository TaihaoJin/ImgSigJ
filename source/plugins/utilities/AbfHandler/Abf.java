...
public class Abf {
	...
	ArrayList <Byte> m_vectcPostDataBuffer;
        boolean m_bReadHeaderOnly;
    String headFileDir;
    int m_nNumChannels;
    String m_sFilePath;
    ArrayList<PlotWindowPlus> m_cvChannelPlots;
    ArrayList<String> m_svChannelNameAndUnits;
...
//	float m_fEpochInitLevel[]=new short[10],m_fEpochLevelInc[]=new short[10],m_fWaveformOffset;
    ...
    public Abf(){
        ...
        m_sFilePath=null;
        m_bReadHeaderOnly=false;
        m_cvChannelPlots=new ArrayList();
    }


    int ReadData (String path) throws FileNotFoundException,IOException
    {
...
//        fread(&m_sUnused1966,80,1,fpAbfIn);
        ...
        for(int k=0;k<m_nADCNumChannels;k++)
        {
            ...
            fScale/=m_fADCDisplayAmplification[k];
...
//                    *(m_pfData+nPosition)=nAmplitude*fScale;
                    m_pfData[nPosition]=nAmplitude*fScale+m_fInstrumentOffset[i0]+m_fSignalOffset[i0];
                    nPosition++;
                }
            }
        }
...
//                    *(m_pfData+nPosition)=fAmplitude;
                    m_pfData[nPosition]=LittleEndianIO.readFloatLittleEndian(ds)+m_fInstrumentOffset[i0]+m_fSignalOffset[i0];;
                    nPosition++;
                }
            }
        }
...
//            fread(&cTemp, sizeof(char),1,fpAbfIn);
    ...
    AbfNode GetTrace()
    {
        ...
        float fAmp;
        float[] pfData;
//        pfData=new float[m_lActualAcqLength];
        pfData=m_pfData;
        int i;
        for(i=0;i<m_lActualAcqLength;i++)
        {
...
//            pfData[i]=fAmp;
        ...
        aNode.nNumEpisodes=m_lActualEpisodes;
        aNode.nSamplesPerEpisode=m_lNumSamplesPerEpisode;
        return aNode;
    }
...
//            fwrite(&cTemp,sizeof(char),1,fpAbfOut);
    ...
    public void importHeader(String path){
        m_bReadHeaderOnly=true;
        importData(path);
        m_bReadHeaderOnly=false;
    }
    ...
    public int exportAsAbf(float[] pfData,int nChannels,int nSamplesPerChannel,String path){
        if(nChannels>16) return -1;
        exportAsAbf(pfData,nChannels,nSamplesPerChannel,1000/nChannels,path);
        return 1;
    }
    public void exportAsAbf(float[] pfData,int nChannels,int nSamplesPerChannel,float fSamplingInterval,String path){
        if(m_nADCNumChannels!=nChannels) loadHeader(nChannels);
        m_pfData=pfData;
        m_lActualAcqLength=nChannels*nSamplesPerChannel;
        ResetSampleInterval(fSamplingInterval);
        autoScale();
        exportData(path);
    }
    public void exportAsAbf(ArrayList<double[]> pdvData,double samplingInterval, String path){
        int nChannels=pdvData.size();
        int nSamplesPerChannel=pdvData.get(0).length;
        int lActualAcqLength=nChannels*nSamplesPerChannel;
        float[]pfData=new float[lActualAcqLength];
        int index=0,i,j;
        for(j=0;j<nSamplesPerChannel;j++){
            for(i=0;i<nChannels;i++){
                pfData[index]=(float)pdvData.get(i)[j];
                index++;
            }
        }
        exportAsAbf(pfData,nChannels,nSamplesPerChannel,(float)samplingInterval,path);
    }
    public void loadHeader(int nNumChannels){
        String defaultHeaderDir=FileAssist.getUserDir()+"\\"+"abf header files\\";
    //     String dir=FileAssist.getUserDir()+"abf header files"+File.separator;
         ...
         String path=defaultHeaderDir+name;
         if(!FileAssist.fileExists(path))
             path=FileAssist.getFilePath("import a pClamp data file for the header. ("+nNumChannels+" channels)", "","axon binary file", "abf", true);
         importHeader(path);
   }
   ...
   public char[][] getADCChannelNames(){
       return m_sADCChannelName;
   }
   public char[][] getADCChannelUnits(){
       return m_sADCUnits;
   }
   public void setChannelName(String sName, int nChannelIndex){
       int maxIndex=m_sADCChannelName.length-1;
       int maxlen=m_sADCChannelName[0].length;
       if(nChannelIndex>maxIndex) IJ.error("The channel index exceeded the maximum channel number");
       int len=sName.length();
       if(len>maxlen) len=maxlen;
       for(int i=0;i<len;i++){
           m_sADCChannelName[nChannelIndex][i]=sName.charAt(i);
       }
   }
   public String getChannelUnit(int index){
       int maxIndex=m_sADCUnits.length-1;
       if(index>maxIndex) IJ.error("The channel index exceeded the maximum channel number");
       return new String(m_sADCUnits[index]);
   }
   public String getChannelName(int index){
       int maxIndex=m_sADCUnits.length-1;
       if(index>maxIndex) IJ.error("The channel index exceeded the maximum channel number");
       return new String(m_sADCChannelName[index]);
   }
   public void setChannelUnit(String sName, int nChannelIndex){
       int maxIndex=m_sADCUnits.length-1;
       int maxlen=m_sADCUnits[0].length;
       if(nChannelIndex>maxIndex) IJ.error("The channel index exceeded the maximum channel number");
       int len=sName.length();
       if(len>maxlen) len=maxlen;
       for(int i=0;i<len;i++){
           m_sADCUnits[nChannelIndex][i]=sName.charAt(i);
       }
   }
   public void setHeaderFileDir(String headerFileDir){
       this.headFileDir=headerFileDir;
   }
   public void showData(){
       int nChannels=m_nADCNumChannels,i;
       obtainChannelNameAndUnits();
       obtainData();
       
       if(m_cvChannelPlots==null) m_cvChannelPlots=new ArrayList();
       int nPlots=m_cvChannelPlots.size();
       
       PlotWindowPlus pw;
       for(i=0;i<nPlots;i++){
           pw=m_cvChannelPlots.get(i);
           pw.close();
       }
//    public PlotWindowPlus(double[] pdX, double[] pdY, String plotTitle, String xTitle, String yTitle, int lw, int shape, Color c){
       for(i=0;i<nChannels;i++){
           pw=new PlotWindowPlus(m_pdTime,m_pdvData.get(i),FileAssist.getFileName(m_sFilePath)+" Channel"+i,"Time (s)",m_svChannelNameAndUnits.get(i),1,2,Color.black);
           m_cvChannelPlots.add(pw);
       }
   }
   public void obtainChannelNameAndUnits(){
       if(m_svChannelNameAndUnits==null){
           m_svChannelNameAndUnits=new ArrayList();
       }
       m_svChannelNameAndUnits.clear();
       int i,j;
       char c;
       String ChannelName="";
       for(i=0;i<m_nADCNumChannels;i++){
           ...
           ChannelName.trim();
           ChannelName+=")";
           m_svChannelNameAndUnits.add(ChannelName);
       }
   }
   public int getNumChannels(){
       return m_nADCNumChannels;
   }
   void obtainData(){
       if(m_pdvData==null)
           m_pdvData=new ArrayList();
       m_pdvData.clear();
       int len=m_lActualAcqLength/m_nADCNumChannels,i,j,index;
       m_pdTime=CommonStatisticsMethods.getEmptyDoubleArray(m_pdTime, len);
       for(i=0;i<m_nADCNumChannels;i++){
           m_pdvData.add(new double[len]);
       }
       for(i=0;i<len;i++){
           m_pdTime[i]=i*m_fADCSampleInterval/1000000;//unit is scond
       }
       double[] pdData;
       for(i=0;i<m_nADCNumChannels;i++){
           pdData=m_pdvData.get(i);
           for(j=0;j<len;j++){
               index=j*m_nADCNumChannels+i;
               pdData[j]=m_pfData[index];
           }
       }
   }
   public int importAtf(String path){
       int[] forms=new int[3];
       AsciiInputAssist.getArrayFormat(path, forms);
       int rows=forms[0],cols=forms[1],channels=cols-1;
       loadHeader(channels);
       
       double[][] pdData=AsciiInputAssist.readAsciiArray(path);
       int len=rows*channels;
       UpdateSize(len);
       PreparePointers();
       ...
       for(r=0;r<rows;r++){
           for(c=1;c<cols;c++){
                m_pfData[position]=(float)pdData[r][c];
                position++;
           }
       }       
       return 1;
