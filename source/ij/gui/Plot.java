...
//    public static final int TOP_MARGIN = 15;
    public int TOP_MARGIN = 15; //changed by TJ on 9/21/2014 to add graph title
    /** the margin width below the plot frame */
//    public static final int BOTTOM_MARGIN = 40;
    public int BOTTOM_MARGIN = 50; //changed by TJ on 9/21/2014 to add graph title
    ...
    boolean showXAxisLabel;//added by Taihao Jin on 10/5/2014
    ...
    public void showXAxisLabel(boolean show){
        showXAxisLabel=show;
    }
    ...
    public Plot(String title, String xLabel, String yLabel, float[] xValues, float[] yValues,int TopMargin,int BottomMargin) {
        this(title, xLabel, yLabel, xValues, yValues, DEFAULT_FLAGS);
    ...
    void drawAxisLabels() {
		...
		ip.drawString(xLabel, LEFT_MARGIN+(frame.width-ip.getStringWidth(xLabel))/2, y+10);
    ...
    void drawTicksEtc() {
        ...
        if ((flags&(X_NUMBERS + X_TICKS + X_GRID)) != 0&&showXAxisLabel) {//modified by Taihao Jin on 10/5/2014
    ...
    void createImage() {
       ...
       int width = plotWidth+LEFT_MARGIN+RIGHT_MARGIN;
        ...
        if(width<=0||height<=0){
            IJ.error("the size of the image must be positive");
        }
    ...
    String getCoordinates(int x, int y) {//x, y are the position of the pixel, it returns the values (curve)
        String text = "";
    ...
    String getPointCoordinates(int x, int y) {// display cursor location
        String text = "";
        if (!frame.contains(x, y))
        return text;
    ...
    public void getScales(double[] scales){//added by TJ on 5/27/2014
        scales[0]=xScale;
        scales[1]=yScale;
    }
    ...
    public Point getPixelCoordinates(double dx, double dy){//added by TJ on 9/21/2011
        int x = LEFT_MARGIN + (int)((dx-xMin)*xScale);
        int y = TOP_MARGIN + frame.height - (int)((dy-yMin)*yScale);
        return new Point (x,y);
    }
    public Rectangle getPlotFrame(){//added by TJ on 9/21/2011
        return new Rectangle(frame);
    }
    public void ReSize(int width, int height) {
        if (width>LEFT_MARGIN+RIGHT_MARGIN+20 && height>TOP_MARGIN+BOTTOM_MARGIN+20) {
            plotWidth = width-LEFT_MARGIN-RIGHT_MARGIN;
            plotHeight = height-TOP_MARGIN-BOTTOM_MARGIN;
        }
        draw();
    }
    public int getPointCoordinates(int x, int y,double[] pdXY) {// display plot value (x and y) at cursor location
        String text = "";
        if (!frame.contains(x, y)){
            pdXY[0]=Double.NaN;
            pdXY[1]=Double.NaN;
    ...
    public void setTopMargin(int mar){//added by TJ on 14921
        plotHeight+=(mar-TOP_MARGIN);
        TOP_MARGIN=mar;
    }
    public void setBottomMargin(int mar){//added by TJ on 14921
        plotHeight+=(mar-BOTTOM_MARGIN);
        BOTTOM_MARGIN=mar;
    }
}
