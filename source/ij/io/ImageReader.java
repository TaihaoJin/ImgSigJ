...
public class ImageReader {
	...
	/**
	Constructs a new ImageReader using a FileInfo 
         * object to describe the file to be read.
	@see ij.io.FileInfo
	*/
	...
	byte[] read8bitImage(InputStream in) throws IOException {
		if (fi.compression == FileInfo.LZW || fi.compression == 
	...
	byte[] readCompressed8bitImage(InputStream in) throws IOException {
		...
		for (int i=0; i<fi.stripOffsets.length; i++) {
			if (in instanceof RandomAccessStream)
				((RandomAccessStream)in).seek(
                                        fi.stripOffsets[i]);
			else if (i > 0) {
				int skip = fi.stripOffsets[i] - 
                                        fi.stripOffsets[i-1] - 
                                        fi.stripLengths[i-1];
			...
			if (fi.compression == FileInfo.LZW_WITH_DIFFERENCING) {
				for (int b=0; b<length; b++) {
					byteArray[b] += last;
					last = b % fi.width == fi.width - 
                                                1 ? 0 : byteArray[b];
				}
			}
			if (current+length>pixels.length) length = 
                                pixels.length-current;
	...
	/** Reads a 16-bit image. Signed pixels are converted to 
         * unsigned by adding 32768. */
		...
		while (totalRead<byteCount) {
			...
			if (fi.intelByteOrder) {
				if (fi.fileType==FileInfo.GRAY16_SIGNED)
					for (int i=base,j=0; 
                                i<(base+pixelsRead); i++,j+=2)
						pixels[i] = (short)((((
                                                        buffer[j+1]&0xff)<<8) | 
                                                       (buffer[j]&0xff))+32768);
				else
					for (int i=base,j=0; i<
                                        (base+pixelsRead); i++,j+=2)
						pixels[i] = (short)(((buffer[j+
                                                        1]&0xff)<<8) | 
                                                        (buffer[j]&0xff));
			} else {
				if (fi.fileType==FileInfo.GRAY16_SIGNED)
	...
	short[] readCompressed16bitImage(InputStream in) throws IOException {
		...
		for (int k=0; k<fi.stripOffsets.length; k++) {
			//IJ.log("seek: "+fi.stripOffsets[k]+" "+
                    //(in instanceof RandomAccessStream));
			if (in instanceof RandomAccessStream)
				((RandomAccessStream)in).seek(
                                        fi.stripOffsets[k]);
			else if (k > 0) {
				int skip = fi.stripOffsets[k] - 
                                    fi.stripOffsets[k-1] - fi.stripLengths[k-1];
				if (skip > 0) in.skip(skip);
			}
	...
	float[] read32bitImage(InputStream in) throws IOException {
		...
		while (totalRead<byteCount) {
			...
			bufferCount = 0;
			while (bufferCount<bufferSize) { // fill the buffer
				count = in.read(buffer, bufferCount, 
                                        bufferSize-bufferCount);
				if (count==-1) {
					if (bufferCount>0)
						for (int i=bufferCount; i<
                                                bufferSize; i++) buffer[i] = 0;
					totalRead = byteCount;
					eofError();
					break;
				}
	...
	float[] read64bitImage(InputStream in) throws IOException {
		...
		while (totalRead<byteCount) {
			...
			for (int i=base; i < (base+pixelsRead); i++) {
				...
				if (fi.intelByteOrder)
					tmp = (long)((b1<<56)|(b2<<48)|(b3<<40)
                                                |(b4<<32)|(b5<<24)|(b6<<16)
                                                |(b7<<8)|b8);
				else
					tmp = (long)((b8<<56)|(b7<<48)|(b6<<40)
                                                |(b5<<32)|(b4<<24)|(b3<<16)
                                                |(b2<<8)|b1);
	...
	short[] read12bitImage(InputStream in) throws IOException {
		...
		for (int index=0; index<buffer.length/3; index++) {
			pixels[j++] = (short)(((buffer[i]&0xff)*16) + 
                                ((buffer[i+1]>>4)&0xf));
			pixels[j++] = (short)(((buffer[i+1]&0xf)*256) + 
                                (buffer[i+2]&0xff));
			i += 3;
		}
		return pixels;
	...
	void skip(InputStream in) throws IOException {
		if (skipCount>0) {
			...
			while (bytesRead<skipCount) {
				...
				bytesRead += count;
				//IJ.log("skip: "+skipCount+" "+count+
                                //" "+bytesRead+" "+skipAttempts);
			}
		}
 ...
 */
		...
		while (true) {
			...
			if (code == CLEAR_CODE) {
				...
				if (code < nextSymbol) {
					...
					out.add(symbolTable[code]);
					// add string to table
					ByteVector symbol = new ByteVector(
                                                byteBuffer1);
					...
					// out of table
					ByteVector symbol = new 
                                                ByteVector(byteBuffer2);
					symbol.add(symbolTable[oldCode]);
	...
	void debug(String label, InputStream in) {
		...
		if (in instanceof RandomAccessStream) {
			try {
				offset = 
         ((RandomAccessStream)in).getFilePointer();
			} catch(Exception e) {}
		}
