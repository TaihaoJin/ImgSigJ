...
public class Menus {
	...
	static void checkSubdirectory_TJ(String path, String dir, Vector v) {
		...
		for (int i=0; i<list.length; i++) {
			String name = list[i];
                        File f1= new File(path+dir1,name);
                        if(f1.isDirectory())
                        {
                            checkSubdirectory_TJ(path,dir1+name,v);
                        }else
                        {
		...
		if (plugins!=null) {
			for (int i=0; i<plugins.length; i++) {
				if (!skipList.containsKey(plugins[i]))
					installUserPlugin_TJ(plugins[i]);
			}
		}
		installJarPlugins_TJ();
		installMacros();
	}
		...
		for (int i=0; i<jarFiles.size(); i++) {
            ...
            try {
				...
				try {if (lnr!=null) lnr.close();}
                        ...
                        boolean bComposedJar=false;
                        if(nEntries>1)bComposedJar=true; //TJ modification
			for (int j=0; j<nEntries; j++) {
				...
				installJarPlugin_TJ(jar, s, bComposedJar);
			}
		}		
	}

	void installUserPlugin_TJ(String className) {
             if (menusTable==null) menusTable = new Hashtable();
             Menu menu = pluginsMenu;
             Menu psubmenu=null;
             int slashIndex = className.lastIndexOf('/');
             String command = className;
             if(slashIndex>0) {//the class files are in the subdirectory of the plugins directory
		...
		//className = className.replace('/', '.');
                 String submenuName=dir;
                 int slashIndex1=dir.lastIndexOf(File.separator);
                 Menu menu1= null;       //upper level submenu;                 
                 Menu menu0= null;       //lower level submenu;
                 psubmenu=null;
                 while(slashIndex1>0)
                 {//the plugin class file is located in lower subdirectory than the immediate subdirectory of the plugins directory
                     submenuName=dir.substring(slashIndex1+1);
                     dir=dir.substring(0, slashIndex1);
                     menu1=(Menu)menusTable.get(submenuName);
                     if (menu1==null) {
                         menu1 = new Menu(submenuName);
                         if (menusTable==null) menusTable = new Hashtable();
                         menusTable.put(submenuName, menu1);
                         if(psubmenu==null)
                         {//This is the lowest level submenu
                             psubmenu=menu1;
                         }else
                         {
                             menu1.add(menu0);
                     ...
                     slashIndex1=dir.lastIndexOf(File.separator);
                }                        
                if(slashIndex1<0)
                {
                   submenuName=dir;
                   ...
                   if (menu1==null) {
                       ...
                       pluginsMenu.add(menu1);
                  }
                        
		//IJ.write(dir + "  " + className);
		}else{//the class files are in the plugins directory
                     psubmenu=pluginsMenu;
                }
    ...
    void installJarPlugin_TJ(String jar, String s, boolean bComposedJar) {
		addSorted = false;
		Menu menu;
        if (s.startsWith("Plugins>")) {
			int firstComma = s.indexOf(',');
			if (firstComma==-1 || firstComma<=8)
				...
				menu = getPluginsSubmenu_TJ(name, jar, bComposedJar);// TJ modification
//				menu = getPluginsSubmenu(name);
			}
        } else if (s.startsWith("\"") || s.startsWith("Plugins")) {
        	String name = getSubmenuName(jar);
                if (name!=null) menu = getPluginsSubmenu_TJ(name, jar, bComposedJar);
                else menu = pluginsMenu;
			addSorted = true;
    ...
    Menu getPluginsSubmenu_TJ(String SubmenuName, String jarPath, boolean bComposedJar) {
	int index0=jarPath.lastIndexOf(File.separatorChar);
	jarPath = jarPath.substring(0,index0);
        if(bComposedJar)
        {
            jarPath=jarPath+File.separator+SubmenuName+"_Ext";
        }
        if (jarPath.startsWith(pluginsPath)){
                        index0=jarPath.lastIndexOf(File.separatorChar);
                        submenuName=jarPath.substring(index0+1);
			jarPath = jarPath.substring(0,index0);
                        ...
                        }else
                        {
                                    ...
                                    addToUperlevelMenu_TJ(jarPath,menu);
                                    return menu;
                        }
        ...
        addItemSorted(menu, menu, userPluginsIndex);
        if (menusTable==null) menusTable = new Hashtable();
        menusTable.put(submenuName, menu);
        //IJ.log("getPluginsSubmenu: "+submenuName);        
        return menu;
	...
	public static synchronized String[] getPlugins_TJ() {
		...
		for (int i=0; i<list.length; i++) {
			...
			if (hasUnderscore && isClassFile && name.indexOf('$')<0 ) {
				...
				if (!isClassFile)
					checkSubdirectory_TJ(pluginsPath, name, v);
			}
		}
        ...
        public void addToUperlevelMenu_TJ(String jarPath, Menu menu0){
        	int index0=jarPath.lastIndexOf(File.separatorChar);
                String submenuName=jarPath.substring(index0+1);
		jarPath = jarPath.substring(0,index0);
