...
public class FileOpener {
	...
	public ImagePlus open(boolean show) {
		...
		switch (fi.fileType) {
			...
			case FileInfo.RGB48_PLANAR:
				...
				if (fi.description!=null) {
					if (fi.description.indexOf(
                                                "mode=color")!=-1)
					mode = CompositeImage.COLOR;
					else if (fi.description.indexOf(
                                                "mode=gray")!=-1)
		...
		if (fi.sliceLabels!=null&&fi.sliceLabels.length==
                        1&&fi.sliceLabels[0]!=null)
	...
	ImagePlus openStack(ColorModel cm, boolean show) {
		...
		if (fi.sliceLabels!=null && 
                        fi.sliceLabels.length<=stack.getSize()) {
	...
	void setStackDisplayRange(ImagePlus imp) {
		...
		for (int i=1; i<=n; i++) {
			IJ.showStatus(
                                "Calculating stack min and max: "+i+"/"+n);
	...
	public void revertToSaved(ImagePlus imp) {
		...
		if (fi.fileFormat==fi.DICOM) {
			// restore DICOM
			ImagePlus imp2 = (ImagePlus)
                                IJ.runPlugIn("ij.plugin.DICOM", path);
			if (imp2!=null)
		...
		if (fi.fileFormat==fi.BMP) {
			// restore BMP
			ImagePlus imp2 = (ImagePlus)IJ.runPlugIn(
                                "ij.plugin.BMP_Reader", path);
			if (imp2!=null)
				imp.setProcessor(null, imp2.getProcessor());
	    	return;
		}
		...
		if (fi.fileFormat==fi.IMAGEIO) {
			...
			if (imp2!=null) imp.setProcessor(null, 
                                imp2.getProcessor());
	    	return;
		}
		...
		switch (fi.fileType) {
			...
			case FileInfo.ARGB:
			case FileInfo.RGB_PLANAR:
	    		img = Toolkit.getDefaultToolkit().createImage(new 
	...
	void setCalibration(ImagePlus imp) {
		if (fi.fileType==FileInfo.GRAY16_SIGNED) {
			...
			coeff[0] = -32768.0;
			coeff[1] = 1.0;
 			imp.getLocalCalibration().setFunction(Calibration.
                                STRAIGHT_LINE, coeff, "gray value");
		}
		...
		if (fi.valueUnit!=null) {
			...
			if ((f>=Calibration.STRAIGHT_LINE && f<=
                                Calibration.RODBARD2 && fi.coefficients!=null)
			|| f==Calibration.UNCALIBRATED_OD) {
				boolean zeroClip = props!=null && props.
                                        getProperty("zeroclip", "false").
                                        equals("true");	
				cal.setFunction(f, fi.coefficients, 
                                        fi.valueUnit, zeroClip);
				calibrated = true;
			}
		}
		...
		if (!(displayMin==0.0&&displayMax==0.0)) {
			...
			if (type==ImagePlus.GRAY8 || type==ImagePlus.COLOR_256)
				...
				if (ip.getMin()!=displayMin || 
                                        ip.getMax()!=displayMax)
					ip.setMinAndMax(displayMin, displayMax);
			}
		}
		...
		if (stackSize>1) {
			...
			//IJ.log("setCalibration: "+channels+"  
                        //"+slices+"  "+frames);
	...
	void checkForCalibrationConflict(ImagePlus imp, Calibration cal) {
		...
		if  (gcal==null || !showConflictMessage || IJ.isMacro())
			return;
		if (cal.pixelWidth==gcal.pixelWidth && 
                        cal.getUnit().equals(gcal.getUnit()))
			return;
	...
	public ColorModel createColorModel(FileInfo fi) {
		if (fi.fileType==FileInfo.COLOR8 && fi.lutSize>0)
			return new IndexColorModel(8, fi.lutSize, 
                                fi.reds, fi.greens, fi.blues);
		else
			return LookUpTable.createGrayscaleColorModel
                                (fi.whiteIsZero);
	}
	...
	public InputStream createInputStream(FileInfo fi) throws IOException, 
		...
		InputStream is = null;
		boolean gzip = fi.fileName!=null && (fi.fileName.endsWith(".gz")
                        ||fi.fileName.endsWith(".GZ"));
		if (fi.inputStream!=null)
			is = fi.inputStream;
		else if (fi.url!=null && !fi.url.equals(""))
			is = new URL(fi.url+fi.fileName).openStream();
		else {
			if (fi.directory.length()>0 && !fi.directory.endsWith
                                (Prefs.separator))
	...
	static boolean validateFileInfo(File f, FileInfo fi) {
		...
		if (fi.fileType==FileInfo.BITMAP || fi.compression!=
                        FileInfo.COMPRESSION_NONE)
			return true;
		length = f.length();
		long size = fi.width*fi.height*fi.getBytesPerPixel();
		size = fi.nImages>1?size:size/4;
		if (fi.height==1) size = 0; // allows plugins to read info of 
                //unknown length at end of file
		if (offset+size>length) {
		   error("Offset + image size > file length.", fi, 
                           offset, length);
		   return false;
		}
		return true;
	...
	/** Reads the pixel data from an image described by 
         * a FileInfo object. */
		...
		try {
			...
			if (!Macro.MACRO_CANCELED.equals(e.getMessage())) {
				...
				if (IJ.getInstance()!=null)
					new ij.text.TextWindow("Exception", 
                                                s, 350, 250);
				else
					IJ.log(s);
			}
